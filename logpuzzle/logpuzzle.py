...
def read_urls(filename):
  ...
  logtext = in_file.read()
  url_dict = {} # store in a url to weed out duplicates

  filenames = re.findall(r'GET (\S+puzzle\S+)', logtext)

  for filename in filenames:
    url_dict['http://' + host + filename] = 1
...
def download_images(img_urls, dest_dir):
  ...
  count = 0
  for img in img_urls:
    filename = 'img%d.jpg' % count
    print 'retrieving ' + img + '...'
    urllib.urlretrieve(img, os.path.join(dest_dir, filename))
